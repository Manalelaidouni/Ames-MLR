...
with st.sidebar:
    ...
    st.markdown('Answer the following questions about your fictional dream house in the real estate market of Ames and find out its price.')
    st.text("")
...
> - Explore the worth of your imaginary dream house in Ames, Iowa using advanced data analysis and modeling techniques.
""")
...
st.markdown("""Let's explore the houses in Ames. In each frame, when you hover over a specific house, the hover text will display the year of construction of that house, along with the neighborhood name on top and the sale price.
...
Home prices in Ames, Iowa  range from \$34,900 to \$755,000 with an average of \$180,921.

- We observe that most houses with a price higher than 500K are built after 1990 in the 'Northridge' and 'NorthRidge Heights' neighborhoods, these neighborhoods are relatively newer and stand out as having the most expensive houses. Other highly priced houses can also be found in 'Stone Brook'.
...
- The average price difference between a house that is rated a quality score of 10 compared to a house that is assigned a rating score of 2 is approximately $600,000. This means that remodeling the material and finish of the house would significantly increase the house value.

- Strangely, the **overall condition** of the house is not correlated with the value of the house.

- **The number of baths in the basement** increase the house price by a small value. However the **number of baths in the entire house** is positively correlated with house price, where a house with 3 baths is worth $100,000 more than houses with a single bath. The takeaway is constructing more baths in the house would significantly increase the house value.
...
st.image(Image.open('images/after_outliers.png'), caption=' Actual vs Fitted plot on the left along with Residuals vs Fitted plot  on the right to visually inspect for the outliers displayed in red before and after their removal.')
st.markdown("")

st.markdown("- In the **Actual vs Fitted plot** (in the bottom left), we plot the actual values of the response variable against the predicted values from the model. The points are scattered perfectly around the diagonal line and not in a non-linear pattern or a curve form, which suggests that the linearity assumption is met.")
st.markdown("- In the **residuals vs fitted plot** (in the bottom right), we plot the residuals  against the predicted values from the model. It looks like the points are scattered randomly around the horizontal 0 line, this further confirms the linearity claim.")
st.markdown("")
...
st.markdown("""
Here is the action plan, since we need to identify and remove problematic predictors, we took into consideration the following factors to make an informed decision on which predictors to drop:

- **Large VIF values**: we computed the Variance Inflation Factor (VIF) values for each predictor and identified those with VIF values greater than 10, which indicate high correlation with other predictors. These predictors were then considered for removal.
...
st.markdown("""
**Statistical significance between the predictor and response variable**: we evaluated the significance of predictors by computing the p-value of the T-test. The t-test statistic help us determine the correlation between the response and the predictor variable. Our goal is to keep only those variables that are significant in predicting the target variable and have a meaningful relationship with it, meanwhile non-significant predictors, which do not contribute significantly to the model, are considered for removal.

**Correlation between each predictor and response variable**: we assessed the correlation between each predictor and the response variable. Predictors with the least association with the response variable are less useful in predicting the outcome and could negatively impact the overall performance of the model, therefore they are also considered for removal.
""")
...
st.markdown("Given the above information, variables we dropped based on VIF value, Pearson correlation, statistical significance and association with target variable are:")

st.markdown("""
- `LotFrontage` has low correlation with `y`, has high VIF value and it's also non-significant.
- `BsmtFinSF2` has low correlation with `y`, has high VIF value and it's also non-significant.
- `BsmtUnfSF` has low correlation with `y`, has high VIF value and it's also non-significant.
- `MiscVal` has low correlation with `y`, has high VIF value and it's also non-significant.
- `LotFrontage` has a high VIF value and it's also non-significant.
- `PoolArea` has low correlation with `y`, has high VIF value and it's also non-significant.
""")

st.markdown("After recalculating the VIF values following the initial removal of predictors with high VIF, we observed that YearRemodAdd, GarageArea, TotalBsmtSF, and GrLivArea still exhibited high VIF values. Consequently, I removed these predictors from the analysis as well.")
...
st.image(Image.open('images/heatmap_after.png'), caption='Pearson Correlation heatmap after removing the problematic predictors above. It shows that there are no pair of predictors with a Pearson correlation greater than 0.5')
...
st.markdown("> *We can conclude that the normality of the residual terms is met.*")
...
st.markdown("""Visually testing for the hetroscedasticity assumption. We use the above plots to check if the residuals suffer from non-constant variance or not :

- **Residuals plot** : it plots the residuals vs index plot, it's used to plot the spread of the residuals and to inspect for a systematic pattern in the spread that could imply the presence of homoscedasticity.
...
st.markdown("""
To confirm this visual inspection, we also ran the Breusch-Pagan test for hetroscedasticity, and the test concluded that the residual variance does not depend on the X variables, confirming the homoscedasticity assumption of the residuals.
""")
...
Autocorrelation occurs when the residuals of a model are dependent of each other, that is the value of one residual is influenced by the value of another residual. If there is a departure from this assumption, it indicates that there is crucial information that the model has not yet captured or has missed.""")


st.markdown("""
We used the Durbin-Watson test to inspect for autocorrelation of the residuals in a linear regression model. 
""")
...
st.markdown("To avoid data leakage, we began by splitting the data into training and validation sets before imputing the missing data. We then examined the distribution of missing data across the training, validation, and testing sets using the following plot where we notice that certain features have missing values across all sets, while others only appear in the testing data with significantly fewer number of missing values.")
...
st.markdown("")
st.markdown("In our evaluation, we also consider the complexity of the reduced models produced from all feature selection methods, Let's compare their complexity :")
st.markdown("")
...
st.markdown("Given the above two charts we notice that: ")

st.markdown("""`Gradient Boosting` achieved the best performance based on AIC and BIC and the second best performance based on Adjusted R² on the validation set. This indicates that the feature subsets selected by `Gradient Boosting` provide a good fit to the model and demonstrate strong predictive performance. These subsets effectively reduce model complexity on the validation data, resulting in a better trade-off between model complexity and fit, this makes sense since `Gradient Boosting` has the smallest model complexity consisting only of 17 features. """)
st.markdown("")
st.markdown("""Furthermore, since the performance evaluation is conducted on a hold out set, it demonstrates that the model built using this subset of features obtained from the embedded method of `Gradient Boosting` exhibits good generalization on unseen data. On the other hand,  `F-Regression` performed the worst on these metrics because it has the largest model size with 170 features and information criterion metrics penalize large models with non-relevant features.
  """)
st.markdown("")
...
st.markdown("")
st.image(Image.open('images/model_comparison.png'), caption="On the left, the models are sorted in descending order based on their cross-validation RMSE scores. On the right, the adjacent table displays the RMSE scores of the models alongside their corresponding standard deviation scores. The standard deviation scores represent the variation in the produced rmse scores across different folds of the cross-validation, where a low standard deviation indicates the model has a consistent performance, meanwhile a low RMSE score indicates the model has better fit on average.")
...
st.markdown("""You can find my implementation of stacking ensemble  in this [link](https://gist.github.com/Manalelaidouni/2a95604980a0c7b2ca6eb7d4e73d4fc3).""")


# Model interpretation
st.markdown("")
st.markdown("")
st.markdown("### Interpreting Model Results")
st.markdown("")
...
The color scale represents the change of feature values, this help us understand how the model prediction change as the values of the feature change.  For example, the dark blue color indicates lower feature values, while the bright yellow color represents higher values.   
...
- Similarly smaller houses (with smaller `LotArea`) significantly negatively impact the predicted house price, meanwhile the larger the house the more it positively impacts house prediction.
...
- For the `YearBuilt` feature, as expected, older houses negatively impact the predicted price for the house, meanwhile  newer houses significantly impact price prediction in a positive manner.


""")
st.markdown("")

st.markdown("""- When looking at`OverallCond`, the overall condition of the house, we can clearly see that the dark blue points extend much further to the left compared to other features. These points correspond to houses with the worst overall condition and they have the most negative SHAP values. This means that **houses with really bad condition have the biggest impact on lowering the house's price**.*
""")
st.markdown("")

st.markdown("""
- Houses with no fireplaces negatively impact the predicted house price whereas **the more fireplaces the house contains the more positive its impact is on the predicted house price**, this is reflected by the yellow line to the right and the dark blue cluster on the left of zero.
...
st.markdown("""
> *Most Houses don’t have an open porch `OpenPorchSF` or wood deck  area `WoodDeckSF` this is why they have a cluster of dark blue points close to zero,  this indicates that **houses that don’t have these facilities negatively impact the predicted price by a small amount, however having these facilities do increase the value of the house, 
            with larger open porch or wooden deck area having a greater positive impact on price**. This is evident from the yellow points extending to the right of the x-axis.*
""")
st.markdown("")
st.markdown("""
...
- `ExterQual_TA`, `MSZoning_RM`,  `GarageType_Detchd`,  `MSSubClass_30` display the same pattern where the dark blue cluster is to the right of the x-axis and the yellow cluster is to the left, this means that houses that have a detached garage or that are located in a Residential Medium Density zone, as well as houses with a quality of the material on the exterior that is rated as Average or Typical, negatively impact the prediction of the price.

- Meanwhile houses that don’t have a built-in central air conditioning `CentralAir_Y` negatively impact the predicted house price, this is reflected by the points extending to the dark blue line to the left, meanwhile houses that do have central air conditioning positively impact the price.
...
> 
>- But here's an interesting finding, having a detached garage from the house reduces its value, it might be that house residents do not prefer going outside to reach their car especially during bad weather and may generally prefer the convenience and the accessibility of an attached garage.
>
>- Also, as mentioned above, lacking a built-in central air conditioning or a fireplace tends to negatively impacts the house value, as well as having a smaller house size.
""")
...
- Also pertaining to the exterior of the house, constructing a wood deck can add both visual appeal and value as proven above.

- Install central air conditioning to provide cooling throughout the entire house.

- Install a new fireplace.

- Building a bathroom in the basement would also increase the property’s value, it would make it more of a comfortable and versatile living space that could serve as a relaxation den, an office space or even a home gym.
